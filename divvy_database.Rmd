...
### Views
I create a view called `vw_trips_enriched` that joins the fact_trips table with all related dimension tables. This view allows me to query descriptive fields (like station names and member types) without manually joining the tables every time, making analysis simpler and faster.
...
This chunk creates a summarized view called `vw_route_counts` that aggregates the number of rides between each start and end station, grouped by user type. It provides an easy way to analyze popular routes and compare travel patterns between members and casual users.
```{sql vw_route_counts}
